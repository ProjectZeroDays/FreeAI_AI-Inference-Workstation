...
export default function Dashboard() {
  return (
    <div className="min-h-screen bg-[#060a18] pt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {[
            ...
            { icon: <Zap className="w-6 h-6" />, label: 'Agents Running', value: '24', color: 'text-yellow-400' },
          ].map((stat, i) => (
            <div key={i} className="p-6 rounded-xl bg-white/5 border border-white/10">
              <div className={`w-12 h-12 rounded-lg bg-white/5 flex items-center justify-center mb-4 ${stat.color}`}>
                {stat.icon}
              </div>
        ...
        <div className="p-8 rounded-2xl bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-white/10 text-center">
          ...
          <Link
            href="/deploy"
            className="inline-flex items-center gap-2 px-6 py-3 bg-primary hover:bg-primary-hover text-white rounded-lg font-semibold transition-all hover:scale-105"
          >
            Go to Deploy Guide
          </Link>
        </div>
