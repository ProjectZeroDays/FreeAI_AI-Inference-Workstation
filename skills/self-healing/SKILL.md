...
- **Active-Context** — optional. Use it if your environment has a meaningful "what was I doing" tag (an active skill, a current task phase, a build stage, an agent role). Skip if not applicable.
- **Area** — free-form. Pick whatever helps future agents find this.
...
- The failure can't be reproduced live but can be reproduced in a sandbox
...
The skill is agent-agnostic. The `.learnings/HEALS.md` format is plain markdown — any agent (Claude Code, Codex CLI, Copilot, Cursor, Aider, ...) can read and write it.
