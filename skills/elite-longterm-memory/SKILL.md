...
description: "Use when Codex needs a durable AI-agent memory system with write-ahead logs, vector search, git notes, local memory files, and optional cloud backup."
---
...
In your Codex memory/search configuration, for example `${CODEX_HOME:-$HOME/.codex}/config.toml` or a project-specific memory config:
...
rm -rf ${CODEX_HOME:-$HOME/.codex}/memory/lancedb/
# Restart the local memory/search service if your environment provides one

# Export Git-Notes
python3 memory.py -p . export --format json > memories.json

# Check memory health
du -sh ${CODEX_HOME:-$HOME/.codex}/memory/
wc -l MEMORY.md
ls -la memory/
```
...
```bash
# Configure the local memory/search integration for your Codex environment
```
...
→ Verify memorySearch or equivalent memory retrieval is enabled in your Codex memory/search config
