...
Complete setup and usage guide for integrating the self-improvement skill with Codex.

## Overview

Codex uses workspace-based prompt injection combined with event-driven hooks. Context is injected from workspace files at session start, and hooks can trigger on lifecycle events.
...
```bash
cp -r self-improving-agent ${CODEX_HOME:-$HOME/.codex}/skills/
```

### 2. Install the Hook (Optional)

Copy the hook to Codex's hooks directory:
...
```bash
mkdir -p ${CODEX_HOME:-$HOME/.codex}/skills/self-improving-agent/.learnings
```
...
Codex provides tools for cross-session communication:
...
```
spawn_agent(task="Research X and report back", label="research")
```
...
```bash
ls ${CODEX_HOME:-$HOME/.codex}/hooks
```
...
3. Run `find ${CODEX_HOME:-$HOME/.codex}/skills -maxdepth 2 -name SKILL.md` to see loaded skills
