...
Add to `~/.codex/settings.json` for global activation:
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "${CODEX_HOME:-$HOME/.codex}/skills/self-improvement/scripts/activator.sh"
          }
        ]
      }
    ]
...
Codex uses the same hook system as Codex. Create `.codex/settings.json`:
...
- Hook scripts run with the same permissions as Codex
- Scripts only output text; they don't modify files or run commands
- Error detector reads `CODEX_TOOL_OUTPUT` environment variable
- Treat `CODEX_TOOL_OUTPUT` as potentially sensitive; do not log or forward it verbatim unless the user explicitly wants that detail
