...
Adapted for Codex (spawn_agent instead of Codex subagent tools)
...
Use `spawn_agent` for each role. Pass a structured prompt:
...
1. **Dispatch implementer sub-agent** via `spawn_agent`
   - Include: full task text, plan file path, TDD constraint
   - Wait for completion announcement

2. **Dispatch spec-reviewer sub-agent** via `spawn_agent`
...
3. **Dispatch code-quality reviewer sub-agent** via `spawn_agent`
