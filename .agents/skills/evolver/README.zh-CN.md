...
在宿主运行时（如 [Codex](https://codex.com)）内运行时，evolver 输出到 stdout 的 `spawn_agent(...)` 文本可以被宿主捕获并触发后续动作。**在独立模式下，这些只是纯文本输出** -- 不会自动执行任何操作。

| 模式 | 行为 |
| :--- | :--- |
| 独立运行 (`node index.js`) | 生成提示词，输出到 stdout，退出 |
| 循环模式 (`node index.js --loop`) | 在守护进程循环中重复上述流程，带自适应休眠 |
| 在 Codex 中 | 宿主运行时解释 stdout 中的指令（如 `spawn_agent(...)`） |
...
| `index.js`（循环恢复） | 崩溃时向 stdout 输出 `spawn_agent(...)` 文本 | 否（纯文本输出；是否执行取决于宿主运行时） |
...
`index.js` 和 `evolve.js` 中的 `spawn_agent(...)` 字符串是**输出到 stdout 的纯文本**，而非直接函数调用。是否被执行取决于宿主运行时（如 Codex 平台）。进化引擎本身不将 `spawn_agent` 作为可执行代码调用。
...
正常。在独立模式下，evolver 生成 GEP 提示词并输出到 stdout。如果你期望它自动应用更改，需要一个宿主运行时（如 [Codex](https://codex.com)）来解释其输出。或者使用 `--review` 模式手动审查和应用每个进化步骤。
...
**Clone 到哪个目录？**
任意目录均可。如果你使用 [Codex](https://codex.com)，建议 clone 到 Codex 工作区内，以便宿主运行时访问 evolver 的 stdout。独立使用时任何位置都行。
