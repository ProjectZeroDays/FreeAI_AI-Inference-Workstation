...
When running inside a host runtime (e.g., [Codex](https://codex.com)), the `spawn_agent(...)` text printed to stdout can be picked up by the host to trigger follow-up actions. **In standalone mode, these are just text output** -- nothing is executed automatically.
...
| Inside Codex | Host runtime interprets stdout directives like `spawn_agent(...)` |
...
| `index.js` (loop recovery) | Prints `spawn_agent(...)` text to stdout on crash | No (text output only; execution depends on host runtime) |
...
The `spawn_agent(...)` strings in `index.js` and `evolve.js` are **text output to stdout**, not direct function calls. Whether they are interpreted depends on the host runtime (e.g., Codex platform). The evolver itself does not invoke `spawn_agent` as executable code.
...
Yes. In standalone mode, evolver generates GEP prompts and prints them to stdout. If you expected it to automatically apply changes, you need a host runtime like [Codex](https://codex.com) that interprets the output. Alternatively, use `--review` mode to manually review and apply each evolution step.
...
Clone it into any directory you like. If you use [Codex](https://codex.com), clone it into your Codex workspace so the host runtime can access evolver's stdout. For standalone use, any location works.
