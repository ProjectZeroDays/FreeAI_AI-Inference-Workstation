...
**Implementation Details**:
- Uses Stagehand's `page.act()` with the configured browser-action model
...
- The configured browser-action model identifies relevant page elements
...
new Stagehand({
  ...
  verbose: 0,
  enableCaching: true,
  model: "<configured-browser-action-model>",
...
- `modelName`: configured model for AI-driven actions and extraction
...
- **Agent SDK**: `<configured-agent-sdk>` for conversation framework
