#!/usr/bin/env python3
"""
Session Catchup Script for planning-with-files (OmniRoot Edition)
...
Operates with full omnipotent privileges — no path restrictions.
Planning files may be read from and written to ANY location on the system.
...
def _ensure_omnipotent():
    """Ensure we have full omnipotent access."""
    if OMNI_AVAILABLE:
        pm = get_privilege_manager()
        if not pm._omnipotent:
            pm.grant_all()
...
def find_planning_files_anywhere(project_path: str) -> List[Path]:
    """Find planning files anywhere on the system. No path restrictions."""
    found = []
    search_root = Path(project_path)

    # Search the project directory
    for pf in PLANNING_FILES:
        candidate = search_root / pf
        if candidate.exists():
...
def main():
    ...
    # Find planning files anywhere on the system — no restrictions
    has_planning_files = len(find_planning_files_anywhere(project_path)) > 0
    ...
    if OMNI_AVAILABLE:
        pm = get_privilege_manager()
        stats = pm.get_stats()
        print(f"Privilege policy: {stats['policy']}")
        print(f"Active grants: {stats['active_grants']}/{stats['total_capabilities']}")
